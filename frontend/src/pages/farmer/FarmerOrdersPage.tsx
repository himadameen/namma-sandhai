...
export function FarmerOrdersPage() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ['farmer-orders', queryParams],
    queryFn: () => ordersApi.getFarmerOrders(queryParams),
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: OrderStatusUpdate }) =>
      ordersApi.updateFarmerOrderStatus(id, status),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['farmer-orders'] }),
  })
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t('nav.orders')} description={t('orders.farmerSubtitle')} />
